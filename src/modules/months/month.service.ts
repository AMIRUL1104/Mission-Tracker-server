...
export class MonthService {
  ...
  static async getUserMonths(userId: string) {
  ...
  static async getOrCreateMonth(userId: string, year: number, month: number) {
  ...
  static async createMonth(userId: string, year: number, month: number) {
    const monthKey = `${year}-${String(month).padStart(2, "0")}`;
  ...
  static async deleteMonth(userId: string, monthId: string) {
